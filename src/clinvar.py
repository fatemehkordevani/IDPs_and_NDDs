...
if __name__ == '__main__':
    ...
    phens_keywords = ['intellectual disability', 'developmental delay', 'neurodevelopmental disorder', 'motor', 'mobility',
                       'movement', 'autis', 'attention deficit', 'retard', 'epilep', 'schizo']

    phens_filtered_list = []
    for kword in phens_keywords:
    ...
    ## exploding df to separate rows for each phen
    # clinvar_ndd.phen.str.split(",")
    clinvar_ndd = clinvar_ndd.assign(phenotypes=clinvar_ndd.phen.str.split(",")).explode('phenotypes').reset_index()
    clinvar_ndd = clinvar_ndd[clinvar_ndd.phenotypes != '']
    clinvar_ndd = clinvar_ndd.drop(columns=['index', 'FDR', 'Groups', 'phen'])
    clinvar_ndd = clinvar_ndd.drop_duplicates()
    ...
    ## adding var_id
    clin_mobidb_ndd['var_id'] = clin_mobidb_ndd.index + 1000
    clin_mobidb_ndd['var_id'] = 'var_' + clin_mobidb_ndd['var_id'].astype(str)
